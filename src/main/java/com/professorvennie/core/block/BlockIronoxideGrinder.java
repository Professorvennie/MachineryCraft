...
public class BlockIronoxideGrinder extends BlockContainer  implements IBookable{
	...
	public BlockIronoxideGrinder(boolean isActive) {
		...
		if(!isActive){
			this.setBlockName(BlockNames.ironOxideGrinderIdle);
			this.setCreativeTab(MachineryCraft.tabMachineryCraft);
		}
		if(isActive){
			this.setLightLevel(0.9F);
			this.setBlockName(BlockNames.ironOxideGrinderActive);
		}
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityironOxideGrinder();
	}
	...
	public void registerBlockIcons(IIconRegister iiconRegister) {
		...
		this.iconFront = iiconRegister.registerIcon(Reference.MOD_ID + ":" + (this.isActive ? "ironoxide_grinder_front_active" : "ironoxide_grinder_front_idle"));
	}
	...
	public Item getItemDropped(int par1, Random random, int par2) {
		return Item.getItemFromBlock(ModBlocks.ironOxideGrinderIdle);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, MachineryCraft.instance, LibGuiIds.GUIID_IRONOXIDE_GRINDER, world, x, y, z);
		}
		return true;
	}
	...
	public static void updateGrinderBlockState(boolean active, World worldObj,
		...
		if (active) {
			worldObj.setBlock(xCoord, yCoord, zCoord, ModBlocks.ironOxideGrinderActive);
		} else {
			worldObj.setBlock(xCoord, yCoord, zCoord, ModBlocks.ironOxideGrinderIdle);
		}
