...
public class BlockGoldoxideGrinder extends BlockContainer {
	...
	public BlockGoldoxideGrinder(boolean isActive) {
		...
		if(!isActive){
			this.setBlockName(LibNames.goldOxideGrinderIdle);
			this.setCreativeTab(MainRegistry.tabMachineryCraft);
		}
		if(isActive){
			this.setLightLevel(0.9F);
			this.setBlockName(LibNames.goldOxideGrinderActive);
		}
	...
	public void registerBlockIcons(IIconRegister iiconRegister) {
		...
		this.iconFront = iiconRegister.registerIcon(LibStrings.MODID+ ":" + (this.isActive ? "goldoxide_grinder_Active" : "goldoxide_grinder_idle"));
	}
